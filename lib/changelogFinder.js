...
async function getRepositoryUrl(packageName) {
    try {
        const {
            repository: { url },
        } = await got(`https://registry.npmjs.org/${packageName}`).json();
        return url
            .replace('git+', '')
            .replace('git:', 'https:')
            .replace('.git', '');
...
async function getChangelog(packageName) {
    ...
    if (!repositoryUrl) console.log(`Repository for package ${packageName} not found`);

    // try all possible location for changelog (with priority)
    const possibleLocations = ['CHANGELOG.md', 'changelog.md', 'History.md', 'HISTORY.md'];
